...
app.post('/api/reviews-image', async (req, res) => {
  ...
  try {
    ...
    await pool.query("INSERT INTO review_images (path) VALUES (?)", [`images/reviews/${filename}`]);
...
app.get('/api/review-images', async (req, res) => {
  try {
    const [rows] = await pool.query("SELECT path FROM review_images ORDER BY created_at DESC");
